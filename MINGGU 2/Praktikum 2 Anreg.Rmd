...
data <- read_excel("C:/Users/darel/Downloads/NSC-2025.xlsx", sheet="Sheet2")
...
Saat ini terdapat 1 peubah Y dan 9 peubah X, misal ingin dilakukan eksplorasi dan analisis terkait peubah X4

```{r}
plot(data$X4, data$Y,
     main = "Akses Sanitasi Layak vs Indeks Pembangunan Manusia",
     xlab = "ASL",
     ylab = "IPM")
```
...
dengan Y adalah indeks pembangunan manusia dan X4 adalah persentase akses sanitasi layak.

Didapatkan nilai $\hat\beta_0 = 60.25436$ artinya nilai dugaan rataan indeks pembangunan manusia ketika presentase akses sanitasi layak bernilai 0 persen adalah 60.254%, akan tetapi nilai 0 tidak ada dalam selang pengamatan presentase akses sanitasi layak sehingga $\hat\beta_0$ tidak dapat didefinisikan lebih lanjut

Nilai $\hat\beta_0 = 0.1828$ artinya nilai dugaan perubahan dugaan rataan Indeks Pembangunan Manusia jika presentase akses sanitasi layak berubah satu tahun adalah sebesar 0.1828%

Koefisien determinasi yang dihasilkan adalah 0.2393 atau 23.93% artinya keragaman persentase penduduk miskin yang dapat dijelaskan oleh persentase akses sanitasi layak adalah sebesar 23.93%, sisanya yaitu 76.07% dijelaskan oleh peubah lain yang tidak dimasukkan dalam model.
